...
pub fn run_test(file: PathBuf, update: bool) -> Result<()> {
    ...
    // 2. Compile to IR
    let project =
        vidra_lang::Compiler::compile(&ast).map_err(|e| anyhow::anyhow!("Compile error: {}", e))?;
    ...
    for scene in &project.scenes {
        ...
        let frame = pipeline
            .render_frame_index(&project, global_frame)
            .map_err(|e| anyhow::anyhow!("{}", e))?;
        ...
        if update {
            ...
            if !snapshot_path.exists() {
                ...
                test_results.push(HtmlTestResult {
                    scene_id: scene.id.to_string(),
                    status: "MISSING",
                    error_msg: "Run with --update to generate".to_string(),
                    reference_img: String::new(),
                    failed_img: String::new(),
                    diff_img: String::new(),
                });
            ...
            match compare_frames(&frame, &reference) {
                Err(e) => {
                    ...
                    println!(
                        "      Rendered output saved to: {}",
                        failed_absolute.display()
                    );
                    ...
                    test_results.push(HtmlTestResult {
                        ...
                        reference_img: reference_path,
                        failed_img: failed_path.clone(),
                        diff_img: diff_path.clone(),
                    });
                }
                Ok(_) => {
...
fn save_frame_as_png(frame: &FrameBuffer, path: &Path) -> Result<()> {
    ...
    let img = ImageBuffer::<Rgba<u8>, _>::from_raw(frame.width, frame.height, frame.data.clone())
        .ok_or_else(|| {
        anyhow::anyhow!("Failed to convert frame to image buffer (size mismatch)")
    })?;
...
fn compare_frames(frame: &FrameBuffer, reference: &RgbaImage) -> Result<()> {
    if frame.width != reference.width() || frame.height != reference.height() {
        anyhow::bail!(
            "Dimensions mismatch: {}x{} vs reference {}x{}",
            frame.width,
            frame.height,
            reference.width(),
            reference.height()
    ...
    if error_percent > MAX_ERROR_PIXELS_PERCENT {
        anyhow::bail!(
            "Exceeded error tolerance: {:.2}% differing pixels (max {:.2}%)",
            error_percent,
            MAX_ERROR_PIXELS_PERCENT
        );
    }
...
fn generate_html_report(base_name: &str, out_dir: &Path, results: &[HtmlTestResult]) -> Result<()> {
    ...
    html.push_str(
        ".status-PASS, .status-PASSED, .status-UPDATED { color: #10B981; font-weight: bold; }",
    );
    html.push_str(
        ".status-FAIL, .status-FAILED, .status-MISSING { color: #EF4444; font-weight: bold; }",
    );
